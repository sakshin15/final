...
_printmin:
    PUSH {LR}               @ store the return address
    LDR R0, =print_string     @ R0 contains formatted string address
    BL printf               @ call printf
    POP {PC}                @ restore the stack pointer and return

_printmax:
    PUSH {LR}               @ store the return address
    LDR R0, =print_string1     @ R0 contains formatted string address
    BL printf               @ call printf
    POP {PC}                @ restore the stack pointer and return
...
print_string:   .asciz      "MINIMUM VALUE = %d\n"
print_string1:   .asciz      "MAXIMUM VALUE = %d\n"
debug_str:
